...
class ParkPathPlanning:
    ...
    def generate_park_scenario(self,sx, sy, gx, gy):    
        ...
        if -math.atan2(0,-1) < computed_angle <= math.atan2(-1,0):
            ...
            ensure_path1 = np.vstack([np.repeat(x_ensure1,ensure_size/0.25), np.arange(y_ensure1-ensure_size,y_ensure1,0.25)[::-1]]).T
            ensure_path2 = np.vstack([np.repeat(x_ensure2,ensure_size/0.25), np.arange(y_ensure2,y_ensure2+ensure_size,0.25)[::-1]]).T
            park_path = self.plan_park_up_right(x_ensure2, y_ensure2)
            park_path = np.vstack([park_path,  self.plan_park_down_right(x_ensure2, y_ensure2)[::-1]])
            ...
            x_ensure1 = x_ensure2 - d - w
            y_ensure1 = y_ensure2 + l + s
            ensure_path1 = np.vstack([np.repeat(x_ensure1,ensure_size/0.25), np.arange(y_ensure1,y_ensure1+ensure_size,0.25)]).T        ### Buralarda değişiklik yapılacak
            ensure_path2 = np.vstack([np.repeat(x_ensure2,ensure_size/0.25), np.arange(y_ensure2-ensure_size,y_ensure2,0.25)]).T        ###
            park_path = self.plan_park_up_left(x_ensure2, y_ensure2)
            park_path = np.vstack([park_path,  self.plan_park_down_left(x_ensure2, y_ensure2)[::-1]]) 
            ...
            park_path = np.vstack([park_path,  self.plan_park_up_left(x_ensure2, y_ensure2)[::-1]])
            print(park_path)                                                                                                            ###
            ...
            x_ensure1 = x_ensure2 + d + w
            y_ensure1 = y_ensure2 - l - s
            ensure_path1 = np.vstack([np.repeat(x_ensure1,ensure_size/0.25), np.arange(y_ensure1,y_ensure1+ensure_size,0.25)]).T
            ensure_path2 = np.vstack([np.repeat(x_ensure2,ensure_size/0.25), np.arange(y_ensure2-ensure_size,y_ensure2,0.25)]).T
            park_path = self.plan_park_down_right(x_ensure2, y_ensure2)
            park_path = np.vstack([park_path,  self.plan_park_up_right(x_ensure2, y_ensure2)[::-1]])
    ...
    def plan_park_up_right(self, x1, y1):       
            ...
            d = 2
            w = 4
            radius = 6.9  # Sonradan değişebilir
            ...
            curve_y = np.array([])
            y = np.arange(y1,y0+1)
            circle_fun = (radius**2 - (y-y0)**2)
            x = (np.sqrt(circle_fun[circle_fun>=0]) + x0-radius)
            y = y[circle_fun>=0]
            choices = x>x0-radius/2
            x=x[choices]
            y=y[choices]
            ...
            y = np.arange(y1,y0+1)
            circle_fun = (radius**2 - (y-y1)**2)
            x = (np.sqrt(circle_fun[circle_fun>=0]) + x1+radius)
            y = y[circle_fun>=0]
            x = (x - 2*(x-(x1+radius)))
            choices = x<x1+radius/2
            x=x[choices]
            y=y[choices]
            curve_x = np.append(curve_x, x[::-1])
            curve_y = np.append(curve_y, y[::-1])
