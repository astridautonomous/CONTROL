...
class BrakeCommandNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            "Node başlatıldı, /astrid/control/brake_cmd, "
            "/astrid/control/throttle_cmd ve steer_cmd dinleniyor."
        )
    ...
    def brake_callback(self, msg):
        ...
        if self.brake_value == 1:
            ...
            # Fren salık ama gaz da yok: araca 0 0 gönder
            power_value = 0
            self.send_i2c(power_value, self.brake_value, self.steer_cmd_right, self.steer_cmd_left)
        elif self.brake_value == 2 and self.throttle_cmd > 0:
            power_value = 50
    ...
    def send_i2c(self, value1, value2, value3, value4):
        try:
            ...
            self.get_logger().info(
                f"I2C ile gönderildi: Paket: {full_packet} "
                f"(power: {value1}, brake: {value2}, steer_L: {value3}, steer_R: {value4})"
            )
