...
class BrakeManager:
    def __init__(self, node: Node, dist_threshold: float, brake_duration: float, wait_duration: float):
...
################################################# STATE VARIABLES ########################################################################
        ...
        self.is_waiting_after_brake = False  # seconds to wait after breaking
        self.stop_start_time = None
        self.wait_start_time = None 
        self.dynamic_mode = 0
        self.traffic_light_state = []
        self.traffic_light_status = 0  # 0: green, 1: red
...
######################################################## POINTS-AREAS ####################################################################333
        ...
        # Park and station lists are separate
        self.brake_points = []
        ...
        # Add station points to the `brake_points` list
        ...
        # Assign park point to `park_point` variable
...
######################################################## OTHER FUNCTIONS ################################################################
    ...
    def manage_brake(self, current_x: float, current_y: float) -> bool:
        ...
        # YENİ: Sadece trafik ışığı alanındaysak bu kontrolü yap
        if self.inside_traffic_light_area(current_x, current_y):
            # Trafik Işığı Kontrolü
            if self.traffic_light_status == 1:  # red
                self.node.get_logger().warn("Traffic light is red inside the area, braking...")
                self.send_control(throttle=0.0, brake=1.0)
                self.is_stopped_at_traffic_light = True
                self.traffic_light_recently_released = False
                return True  # Frenleme yapıldığı için fonksiyondan çık
            elif self.traffic_light_status == 0:  # green
                if self.traffic_light_recently_released:
                    ...
                    if elapsed_since_release < self.traffic_light_release_delay:
                        self.node.get_logger().info(f"Traffic light is green but waiting for another {self.traffic_light_release_delay - elapsed_since_release:.2f} seconds")
                        self.send_control(throttle=0.0, brake=1.0)
                        self.is_stopped_at_traffic_light = True
                        return True # Frenleme yapıldığı için fonksiyondan çık
                    else:
            ...
            # Trafik ışığı alanının dışındaysak, durma durumunu sıfırla
            if self.is_stopped_at_traffic_light:
                 self.is_stopped_at_traffic_light = False

        # Dinamik Engel Kontrolü (Her zaman aktif)
        if self.dynamic_mode == 1:
            self.node.get_logger().warn("Dynamic obstacle detected, braking...")
            self.send_control(throttle=0.0, brake=1.0)
        ...
        if self.park_point and not self.has_braked_at_park:
            ...
            if abs(current_x - px) < self.dist_threshold and abs(current_y - py) < self.dist_threshold:
                self.node.get_logger().warn("Reached park point, braking...")
                self.send_control(throttle=0.0, brake=1.0)
                self.has_braked_at_park = True
                return True

        # Stations (Diğer durma senaryoları)
        if not brake_applied and not self.is_stopped_at_traffic_light and not self.is_stopped_for_dynamic:
            if self.current_stop_index < len(self.brake_points):
                ...
                if (abs(current_x - tx) < self.dist_threshold and abs(current_y - ty) < self.dist_threshold and not self.has_braked):
        ...
        # Eğer hiçbir fren veya bekleme durumu aktif değilse, gaz ver
...
class BrakeNode(Node):
    def __init__(self):
        ...
        self.brake_manager = BrakeManager(self, dist_threshold=1.0, brake_duration=3.0, wait_duration=5.0) 
