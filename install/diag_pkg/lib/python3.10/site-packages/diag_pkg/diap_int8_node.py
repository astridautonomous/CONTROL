...
=====================================================
Belirtilen tüm topic'lere subscribe olur.
Herhangi biri TIMEOUT süresi içinde yayın yapmazsa
terminale renkli ERROR mesajı basar.
 
Kullanım:
  ros2 run <paket_adı> topic_watchdog_ros2
  ros2 run <paket_adı> topic_watchdog_ros2 --ros-args -p timeout:=3.0
 
Parametreler:
  timeout      : Kaç saniye yayın gelmezse hata verilsin (varsayılan: 5.0)
  check_rate   : Kontrol sıklığı Hz cinsinden (varsayılan: 1.0)
 
Not:
  ROS 2 Humble+ üzerinde create_generic_subscription() kullanılır.
  Topic tipi otomatik keşfedilir; henüz keşfedilemezse tekrar denenir.
"""
        ...
        for topic in pending:
            ...
            try:
                ...
                self.get_logger().debug(
                    f"[TopicWatchdog] Abone olundu: {topic} [{Int8}]"
                )
    ...
    def _print_summary(self):
        ...
        with self._lock:
            for topic in TOPICS:
                last = self._last_received[topic]
                if topic not in self._subscribed_topics:
                    status = f"{RED}⚠  Henüz keşfedilemedi{RESET}"
                elif last is None:
